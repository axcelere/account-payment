    ...
    def _process_order(self, order, existing_order):
        ...
        if order.get('payment_ids'):
            ...
            if instalment:
                ...
                _logger.info(f"✅ Found installment_id in ticket: {installment_id_from_ticket}")
                ...
                if installment.exists() and payment_method_id.instalment_product_id:
                    ...
                    _logger.info(f"📊 Original total: {original_total}, Surcharge coefficient: {installment.surcharge_coefficient}")
                    _logger.info(f"💰 Surcharge total with tax: {surcharge_total_with_tax}")
                    ...
                    _logger.info(f"💵 Calculated price_unit (without tax): {price_unit}")
                    _logger.info(f"🏷️ Tax IDs: {tax_ids}")
                    ...
                    _logger.info(f"✅ Added surcharge line for installment: {installment.name}")
                    _logger.info(f"📈 Updated totals - Tax: {order['amount_tax']}, Total: {order['amount_total']}")
                    _logger.info(f"💳 Updated payment amount: {new_payment_amount}")
