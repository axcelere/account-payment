    ...
    def check(self):
        ...
        if self.card_type in ["credit", "debit"]:
            ...
            if not float_is_zero(
                init_data["amount"], precision_rounding=currency.rounding
            ):
                if self.fee:
                    untax_amount = self.fee
                    if len(self.instalment_id.product_id.taxes_id):
                        tax_computed = (
                            self.instalment_id.product_id.taxes_id.compute_all(self.fee)
                        )
                            ...
                            "lines": [
                                (
                                    ...
                                    0,
                                    {
                                        ...
                                        "name": _("%s %s")
                                        % (
                                            self.instalment_id.product_id.display_name,
                                            self.instalment_id.name,
                                        ),
                                    },
                                )
                            ]
                ...
                order.add_payment(
                    {
                        ...
                        "amount": currency.round(init_data["amount"] + init_data["fee"])
                        if currency
                        else init_data["amount"] + init_data["fee"],
